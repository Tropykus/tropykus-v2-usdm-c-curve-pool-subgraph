...
export function handleTokenExchange(event: TokenExchangeEvent): void {
	...
	if (boughtToken == null) {
		...
		if (boughtTokenContract.symbol().includes("USDC"))
			boughtToken.symbol = "USDC";
	...
	entity.isWithdrawal = !entity.isDeposit;

	if (boughtToken.symbol == "USDM") {
		entity.isDeposit = true;
		entity.isWithdrawal = !entity.isDeposit;
	}

	let userActivity = UserActivity.load(event.params.buyer.toHexString());
	if (userActivity == null) {
		userActivity = new UserActivity(event.params.buyer.toHexString());
		// Interest calculation
		userActivity.cleanDepositsTimestamp = 0;
		userActivity.depositsAfterCleanTimestamp = zeroBI();
		userActivity.withdrawsAfterCleanTimestamp = zeroBI();
		userActivity.bruteDeposits = zeroBI();
		userActivity.save();
	}
  entity.userActivity = userActivity.id;
  entity.save();

	if (userActivity.cleanDepositsTimestamp == 0)
		userActivity.cleanDepositsTimestamp = event.block.timestamp.toI32();

	if (entity.isDeposit)
		userActivity.depositsAfterCleanTimestamp =
			userActivity.depositsAfterCleanTimestamp.plus(entity.tokens_bought);
	else
		userActivity.withdrawsAfterCleanTimestamp =
			userActivity.withdrawsAfterCleanTimestamp.plus(entity.tokens_bought);

	if (
		userActivity.depositsAfterCleanTimestamp.gt(
			userActivity.withdrawsAfterCleanTimestamp
		)
	)
		userActivity.bruteDeposits = userActivity.depositsAfterCleanTimestamp.minus(
			userActivity.withdrawsAfterCleanTimestamp
		);
	else log.error("bruteDeposits is negative", []);

  if((userActivity.bruteDeposits).toBigDecimal().lt(BigDecimal.fromString('10000')) ) {
    userActivity.bruteDeposits = zeroBI();
    userActivity.depositsAfterCleanTimestamp = zeroBI();
    userActivity.withdrawsAfterCleanTimestamp = zeroBI();
    userActivity.cleanDepositsTimestamp = event.block.timestamp.toI32();
